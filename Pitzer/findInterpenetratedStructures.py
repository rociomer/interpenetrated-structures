...
# check for overlap between atoms; return True if no overlap, False otherwise.
    ...
    # newlist3 and newlist4 are boundary atoms
...
cos(gamma)**2 + 2 * cos(alpha) * cos(beta) * cos(gamma)) / sin(gamma) * c
    ...
    for line3 in newlist3:
        for line4 in newlist4:
            if (float(line4[2]) - float(line3[2]))**2 + (float(line4[3]) - \
float(line3[3]))**2 + (float(line4[4]) - float(line3[4]))**2 \
< 4 * overlapRadius**2:
                print "Boundary overlap:", \
(float(line4[2]) - float(line3[2]))**2 + (float(line4[3]) - \
float(line3[3]))**2 + (float(line4[4]) - float(line3[4]))**2
                print line4[0], line4[2], line4[3], line4[4], line3[0], \
line3[2], line3[3], line3[4]
                return False
    # check for regular overlaps.
    for line in newlist1:
        for line2 in newlist2:
            if (float(line2[2]) - float(line[2]))**2 + (float(line2[3]) - \
float(line[3]))**2 + (float(line2[4]) - float(line[4]))**2 \
< 4 * overlapRadius**2:
                print "Regular overlap:", \
(float(line2[2]) - float(line[2]))**2 + (float(line2[3]) - float(line[3]))**2 \
+ (float(line2[4]) - float(line[4]))**2
                print line2[0], line2[2], line2[3], line2[4], line[0], \
...
#  2 * overlapRadius, the structure is rejected
    ...
    n_b = trialIndices[1]
    n_c = trialIndices[2]
    trialStructure = f[:-5] + "_trial_" + str(n_a) + "_" + str(n_b) + "_" + \
str(n_c) + ".cssr"
    newCoordinates = coordTranslation(shiftDistance / \
float(coordinatesArray[0][0]) * n_a, shiftDistance / \
float(coordinatesArray[0][1]) * n_b, shiftDistance / \
float(coordinatesArray[0][2]) * n_c, coordinatesArray[4:]) 
    if checkOverlap(overlapRadius, newCoordinates, coordinatesArray[4:], \
float(coordinatesArray[0][0]), float(coordinatesArray[0][1]), \
float(coordinatesArray[0][2]), radians(float(coordinatesArray[1][0])), \
radians(float(coordinatesArray[1][1])), radians(float(coordinatesArray[1][2]))):
        print "No overlap detected for 2-fold interpenetrated trial structure \
with indices " + str(trialIndices) + "."
        with open(outputfile, 'a') as text: 
            text.write("No overlap detected for 2-fold interpenetrated trial \
structure with indices " + str(trialIndices) + "\n")
        ...
        print "Overlap detected for 2-fold interpenetrated trial structure \
with indices " + str(trialIndices) + "."
        with open(outputfile, 'a') as text: 
            text.write("Overlap detected for 2-fold interpenetrated trial \
structure with indices " + str(trialIndices) + "\n")
        return False

def generateHigherLevelInterpenetrated(shiftDistance, minIndices, \
maxLatticeVector, overlapRadius, coordinatesArray): 
...
#  structure with minimum displacement vectors
    ...
    n_c = minIndices[2]
    newCoordinates = list()
    for n in range(1, int(ceil(maxLatticeVector / (shiftDistance * \
(n_a**2 + n_b**2 + n_c**2)**0.5)))):
        higherLevelIndices_a = n_a * n
        higherLevelIndices_b = n_b * n
        higherLevelIndices_c = n_c * n
        higherLevelIndices = [higherLevelIndices_a, higherLevelIndices_b, \
higherLevelIndices_c]
        newCoordinates += coordTranslation(shiftDistance / \
float(coordinatesArray[0][0]) * higherLevelIndices_a, shiftDistance / \
float(coordinatesArray[0][1]) * higherLevelIndices_b, shiftDistance / \
float(coordinatesArray[0][2]) * higherLevelIndices_c, coordinatesArray[4:]) 
        if checkOverlap(overlapRadius, newCoordinates, coordinatesArray[4:], \
float(coordinatesArray[0][0]), float(coordinatesArray[0][1]), \
float(coordinatesArray[0][2]), radians(float(coordinatesArray[1][0])), \
radians(float(coordinatesArray[1][1])), radians(float(coordinatesArray[1][2]))):
            print "No overlap detected for " + str(n+1) + "-fold \
interpenetrated trial structure with indices " + str(higherLevelIndices) + "!"
            with open(outputfile, 'a') as text: 
                text.write("No overlap detected for " + str(n + 1) + "-fold \
interpenetrated trial structure with indices " + str(higherLevelIndices) + "!\n")
            coordinatesArray[2][0] = str(int(coordinatesArray[2][0])*(n+1))
            higherLevelInterpStructure = f[:-5] + "_interp_" + str(n+1) + \
"_indices_" + str(higherLevelIndices_a) + "_" + str(higherLevelIndices_b) + \
"_" + str(higherLevelIndices_c) + ".cssr"
            with open(higherLevelInterpStructure, 'w') as text:
                for line in coordinatesArray: # non-coordinate information
                    text.write(' '.join(line) + "\n")
                coordinatesArray[2][0] = str(int(coordinatesArray[2][0]) / \
(n + 1))
                for index, line in enumerate(newCoordinates):
                    line[0] = str(index + 1 + int(coordinatesArray[2][0]))
                    text.write(' '.join(line) + "\n")
            ...
            print "Overlap detected for " + str(n + 1) + "-fold \
interpenetrated trial structure with indices " + str(higherLevelIndices) + "."
            with open(outputfile, 'a') as text: 
                text.write("Overlap detected for " + str(n + 1) + "-fold \
interpenetrated trial structure with indices " + str(higherLevelIndices) + "\n")
...
text.read().split('\n')) 
    ...
    t0 = time.time()
    with open(outputfile, 'a') as text: 
        text.write("Generating interpenetrated structures and checking \
overlap\n")
    minIndices = [int(ceil(latticeVector_a / shiftDistance)), \
int(ceil(latticeVector_b / shiftDistance)), int(ceil(latticeVector_c / \
shiftDistance))]
    minIndicesMagnitude = sqrt((minIndices[0] * shiftDistance)**2 + \
(minIndices[1] * shiftDistance)**2 + (minIndices[2] * shiftDistance)**2)
    for n_c in range(int(ceil(latticeVector_c / shiftDistance))):
        for n_b in range(int(ceil(latticeVector_b / shiftDistance))):
            for n_a in range(int(ceil(latticeVector_a / shiftDistance))):
                ...
                IndicesMagnitude = sqrt((n_a * shiftDistance)**2 + (n_b * \
shiftDistance)**2 + (n_c * shiftDistance)**2)
                if IndicesMagnitude > minIndicesMagnitude:
                    break
                elif (IndicesMagnitude == minIndicesMagnitude) and \
(generateInterpenetrated(shiftDistance, trialIndices, overlapRadius, \
coordinatesArray) == True):
                    with open(outputfile, 'a') as text:
                        text.write("Same magnitude of displacement for trial \
structures " + str(trialIndices) + " and " + str(minIndices) + " (magnitude: " \
+ str(IndicesMagnitude) + " A) \n")
                        minIndices = list(trialIndices)
                        minIndicesMagnitude = sqrt((minIndices[0] * \
shiftDistance)**2 + (minIndices[1] * shiftDistance)**2 + (minIndices[2] * \
shiftDistance)**2)
                elif (IndicesMagnitude < minIndicesMagnitude) and \
(generateInterpenetrated(shiftDistance, trialIndices, overlapRadius, \
coordinatesArray) == True):
...
shiftDistance)**2)
                    with open(outputfile, 'a') as text:
                        text.write("New indices for best trial structure " + \
str(minIndices) + " \n")
                        text.write("minIndicesMagnitude " + \
str(minIndicesMagnitude) + " \n")
                else:
                    pass
    t1 = time.time()
    with open(outputfile, 'a') as text: 
        text.write("Time elapsed for finding the best 2-fold interpenetrated \
structure: " + str((t1 - t0) / 60) + " min\n")
    if minIndices != [int(ceil(latticeVector_a)), int(ceil(latticeVector_b)), \
int(ceil(latticeVector_c))]:
        with open(outputfile, 'a') as text: 
            text.write("Generating higher-level interpenetrated structures and \
checking overlap\n")
        maxLatticeVector = max(latticeVector_a, latticeVector_b, \
latticeVector_c)
        generateHigherLevelInterpenetrated(shiftDistance, minIndices, \
maxLatticeVector, overlapRadius, coordinatesArray)
    else:
        print "No successful interpenetrated structures found for " + str(f) + \
"\n"
        with open(outputfile, 'a') as text: 
            text.write("No successful interpenetrated structures found for " + \
str(f) + "\n")
    with open(outputfile, 'a') as text:
        text.write("Program complete " + \
str(time.strftime("%A %d %B %Y, %I:%M%p")) + "\n")
